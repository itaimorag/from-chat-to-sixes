...
import type { Player, Card as CardType, Room } from "@/lib/types";
...
import { RotateCcw, PartyPopper, Layers, Shuffle } from "lucide-react";
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          Each player has 6 cards in two rows. Your bottom row is shown face up
          at the start. Others are face down.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {players.map((player) => {
            ...
            const showBottomRow =
              peekPhase && isCurrentPlayer && !bottomRowPeeked;
            console.log(
              "showBottomRow",
              showBottomRow,
              peekPhase,
            ...
            const canClickForReplacement =
              isCurrentPlayer && !peekPhase && drawnPile !== null;

            return (
              <div
                key={player.id}
                className="p-4 border rounded-lg bg-card/50 relative"
              >
                ...
                <div className="flex flex-col gap-2">
                      ...
                      return (
                        <div
                          ...
                          className={`cursor-pointer transition-transform hover:scale-105 ${
                            canClickForReplacement
                              ? "hover:ring-2 hover:ring-primary"
                              : ""
                          }`}
                          onClick={
                            canClickForReplacement
                              ? () => onReplaceCard?.("top", idx, drawnPile!)
                              : undefined
                          }
                        >
                          {gameOver ? (
                  ...
                  <div className="flex gap-2 justify-center">
                    {(player.hand.bottom || []).map((card, idx) => {
                      const canClickBottomForReplacement =
                        canClickForReplacement && player.canReplaceBottom;

                      return (
                        <div
                          key={idx}
                          className={`cursor-pointer transition-transform hover:scale-105 ${
                            canClickBottomForReplacement
                              ? "hover:ring-2 hover:ring-primary"
                              : ""
                          }`}
                          onClick={
                            canClickBottomForReplacement
                              ? () => onReplaceCard?.("bottom", idx, drawnPile!)
                              : undefined
                          }
                        >
                  ...
                  {showBottomRow && isCurrentPlayer && (
                    <div className="flex justify-center mt-2">
                      <Button onClick={onDonePeek} variant="outline">
                        Done
                      </Button>
                    </div>
                  )}
...
export function GameBoard({
  ...
  const currentRound = Math.max(
    0,
    ...room.players.map((p: Player) => p.scoresByRound.length)
  );
  ...
  const currentPlayer = room.players.find(
    (p: Player) => p.id === room.currentTurnPlayerId
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <PlayerHandsDisplay
        ...
        bottomRowPeeked={player ? player.hasPeeked : true}
        gameState={room.gameState}
        onDonePeek={handlePeekDone}
          ...
          {!drawnPile && (
            <div className="flex gap-4">
              ...
              <Button
                ...
                disabled={room.discard.length === 0}
              >
                Draw from Discard
              </Button>
            </div>
          ...
          {drawnPile && (
            <div className="flex flex-col items-center gap-4">
              <div className="flex flex-col items-center">
                ...
                <Image
                  src={getCardImageSrc(
                    drawnPile === "deck" ? room.deck[0] : room.discard[0]
                  )}
                  alt={
                    drawnPile === "deck"
                      ? room.deck[0].rank + " of " + room.deck[0].suit
                      : room.discard[0].rank + " of " + room.discard[0].suit
                  }
                  width={70}
                  height={100}
                  className="rounded shadow-md"
              ...
              <div className="flex flex-col gap-2">
                <span className="font-semibold text-center">
                  Click on a card in your hand to replace it, or discard the
                  drawn card:
                </span>
                <Button
                  onClick={handleDiscardCard}
                  variant="outline"
                  disabled={drawnPile === "discard"}
                >
                  Discard Drawn Card
                </Button>
              </div>
      ...
      {isMyTurn && room.gameState === "playing" && drawnPile === null && (
        <div className="flex justify-center mt-4">
          <Button onClick={onCallStop} variant="destructive">
            Call STOP
          </Button>
        </div>
      )}
      ...
      <Scoreboard
        ...
        stopperId={room.stopperId ? room.stopperId : null}
      />
      {room.players.length > 0 && (
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              Track the cards in play. This information is used by the AI
              Advisor. Deck count is automatically updated after dealing from a
              52-card deck.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1">
              <Label htmlFor="cardsRemainingInDeck" className="font-medium">
            ...
            <div className="space-y-1">
              <Label htmlFor="cardsInDiscardPile" className="font-medium">
              ...
              <p className="text-2xl font-semibold">
                {(() => {
                  const winner = room.players.reduce(
                    (prev: Player, curr: Player) =>
                      curr.totalScore < prev.totalScore ? curr : prev,
                    room.players[0]
                  );
                  return winner
                    ? `${winner.name} is the winner with ${
                        winner.scoresByRound[winner.scoresByRound.length - 1]
                      } points!`
                    : "Scores are final!";
                })()}
              </p>
